  ...
  <template>
    <section class="whisper-native" data-view={{this.data.view}} aria-busy={{if this.busy "true" "false"}} {{this.mount}}>
      ...
      {{#if (eq this.data.view "feed")}}
        <div class="whisper-list-controls"><div class="whisper-sort"><button type="button" class="btn btn-flat {{if (eq this.data.sort 'new') 'is-selected'}}" {{on "click" (fn this.sort "new")}}>最新发表</button><button type="button" class="btn btn-flat {{if (eq this.data.sort 'active') 'is-selected'}}" {{on "click" (fn this.sort "active")}}>最新回复</button></div><form class="whisper-search" {{on "submit" this.search}}><input type="search" name="q" value={{this.data.q}} aria-label="搜索树洞" placeholder="搜索内容或树洞编号" maxlength="200" /><button class="btn btn-flat" type="submit" aria-label="搜索">{{dIcon "magnifying-glass"}}</button></form></div>
        <p class="whisper-anonymity-note">仅认证成员可见 · 帖内固定别名 · 查看身份须公开审计</p>
      {{/if}}
      {{#if (eq this.data.view "mine")}}<div class="whisper-subnav"><button class="btn btn-flat {{if (eq this.data.part 'posts') 'is-selected'}}" type="button" {{on "click" (fn this.part "posts")}}>我的树洞</button><button class="btn btn-flat {{if (eq this.data.part 'replies') 'is-selected'}}" type="button" {{on "click" (fn this.part "replies")}}>我的回复</button><a class="btn btn-flat" href="/whisper/my-data">导出我的数据</a></div>{{/if}}
      {{#if this.isList}}
        <table class="topic-list whisper-topic-list"><thead><tr><th scope="col">话题</th><th scope="col" class="num">回复</th><th scope="col" class="num">赞</th><th scope="col" class="activity">活动</th></tr></thead><tbody class="topic-list-body">{{#each this.data.rows key="id" as |row|}}<tr class="topic-list-item" data-row-id={{row.id}}><td class="main-link"><div class="whisper-topic-title">{{#if row.url}}<a class="title" href={{row.url}} {{on "click" this.visit}}>{{row.title}}</a>{{else}}<span>{{row.title}}</span>{{/if}}{{#if row.mine}}<span class="whisper-mine">你的树洞</span>{{/if}}{{#if (eq row.status "hidden")}}<span class="whisper-status">已隐藏</span>{{/if}}</div><p class="whisper-excerpt">{{row.excerpt}}</p>{{#if row.images}}<div class="whisper-feed-images">{{#each row.images as |url|}}<img src={{url}} alt="树洞配图" loading="lazy" />{{/each}}</div>{{/if}}<div class="whisper-topic-meta">{{#if row.public_code}}<span>#{{row.public_code}}</span>{{/if}}<DRelativeDate @date={{row.created_at}} />{{#if row.missing_media_count}}<span>旧版有缺失配图</span>{{/if}}{{#if row.comment}}{{#if row.can_delete}}<button class="btn btn-flat" type="button" aria-label="删除我的回复" {{on "click" (fn this.deleteComment row)}}>{{dIcon "trash-can"}}</button>{{/if}}{{/if}}</div></td><td class="num posts">{{row.replies}}</td><td class="num likes">{{row.likes}}</td><td class="activity"><DRelativeDate @date={{row.activity_at}} /></td></tr>{{/each}}</tbody></table>
      ...
      {{else if (eq this.data.view "post")}}
        <div class="whisper-topic-heading"><a href="/whisper" {{on "click" this.visit}}>{{dIcon "chevron-left"}} 返回树洞广场</a><h2>{{this.data.post.title}}</h2><div class="whisper-topic-meta"><span>#{{this.data.post.public_code}}</span><span>{{this.data.post.replies}} 条回复</span></div></div>
        ...
        {{#if this.data.audits}}<section class="whisper-audits"><h3>{{dIcon "eye"}} 身份查看记录</h3>{{#each this.data.audits key="id" as |audit|}}<p><strong>{{audit.moderator}}</strong> · <DRelativeDate @date={{audit.created_at}} /></p><p>{{audit.reason}}</p>{{/each}}</section>{{/if}}
      ...
      {{else if (eq this.data.view "admin")}}
        ...
        <div class="whisper-admin-list">{{#each this.data.rows key="id" as |row|}}<article class="whisper-admin-row">{{#if row.url}}<a href={{row.url}} {{on "click" this.visit}}>{{if row.title row.title "打开对应内容"}}</a>{{/if}}<p>{{row.excerpt}}{{row.body}}</p>{{#if row.status}}<small>{{statusText row.status}}</small>{{/if}}{{#if row.reason}}<p>{{row.reason}}</p>{{/if}}{{#if row.moderator}}<span>{{row.moderator}} · {{row.action}}</span>{{/if}}{{#if row.username}}<strong>{{row.username}}</strong>{{/if}}<DRelativeDate @date={{row.created_at}} />{{#if (eq this.data.part "reports")}}{{#unless this.data.readonly}}<button type="button" class="btn" {{on "click" (fn this.resolve row)}}>标记已处理</button>{{/unless}}{{/if}}</article>{{else}}<p class="whisper-empty">没有相关记录</p>{{/each}}</div>
      {{/if}}
